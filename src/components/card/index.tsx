    ...
    <div className='shadow-md group rounded overflow-hidden'>
      <div className="relative h-[400px] w-full bg-[#DDDDDD] overflow-hidden">
          ...
          <div onClick={() => handleHeartProduct(product)} className={`cursor-pointer w-8 h-8 rounded-[50%] bg-white absolute ${ isHeart ? " top-4 right-4" : "-top-10 -right-10"} duration-300 flex items-center justify-center group-hover:top-4 group-hover:right-4`}>
              {
               isHeart ? <GoHeartFill size={20} color='#E4A16F' /> : <GoHeart size={20}/>
              }
          ...
          {
            isCart ? 
                <div className="grid grid-cols-3 place-items-center w-[80%] h-10  absolute bottom-4  left-[50%] -translate-x-[50%] shadow-lg rounded-lg btn">
             ...
             : 
                <button onClick={() => dispatch(addToCart(product))} className={`py-2 px-16 w-[80%]  shadow-lg rounded-lg text-white absolute bottom-4 left-[50%] -translate-x-[50%] origin-bottom ${isCart ? "scale-y-100" : "scale-y-0"}  transition-transform duration-300 group-hover:scale-y-100 active:scale-[0.95] hover:bg-[#1a1e20] btn`}>Add to cart</button>
